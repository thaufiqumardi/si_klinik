...
$menusid = $this->M_crud->get_by_param("menu", 'name', "DataPengeluaran");
if(!empty($menusid)){
    			...
    			<div class="box box-widget">
					<div class="box-header with-border">
					  <h3 class="box-title">Data Pengeluaran</h3>
						...
						<div class="box-body">
							  <a href="<?php echo site_url('Keuangan/cetak_pengeluaran');?>" target="_blank" class="btn btn-default pull-right"><i class="fa fa-print"></i> Cetak</a>
			              	  <a href="<?php echo site_url('Keuangan/doexport_pengeluaran');?>" style="margin-right: 10px;" target="_blank" class="btn btn-success pull-right">
			              		<i class="fa fa-file-excel-o"></i> Export Excell</a>
							  <?php if($this->session->userdata['simklinik']['ap_is_admin'] == 1 || $mnCreate == 1){
			              	  ?>
			              	  <a href="<?php echo base_url(); ?>Keuangan/add_pengeluaran" style="margin-right: 10px;" class="btn btn-primary pull-right">
						  ...
						  <table id="example2" class="table table-bordered table-striped">
							...
							<?php
								if($pengeluaran > 0)
								{
									...
									foreach($pengeluaran as $data)
									{
										...
										$total_pengeluaran = $this->M_base->currFormat2($data->total_pengeluaran);
										$total_pengeluaran = str_replace(".00", "", $total_pengeluaran);
										$total_pengeluaran = "Rp. ".$total_pengeluaran;
							?>
								<tr class="odd gradeX">
									<td><?= $no ?></td>
									<td><?= $data->no_pengeluaran ?></td>
							...
							<?php
								...
								{
							?>
									<a class="btn btn-warning btn-xs" href="<?php echo site_url('keuangan/edit_pengeluaran\/').$data->pengeluaran_id;?>">
							...
							?>
									<a class="btn btn-danger btn-xs" title="Hapus" href="" data-toggle="modal" 
										onclick="confirm_delete('<?php echo site_url('keuangan/delete_pengeluaran').'/'.$data->pengeluaran_id;?>',
										'<?php echo $data->no_pengeluaran;?>');"><i class="fa fa-trash"></i></a>
							<?php
								}
							?>
