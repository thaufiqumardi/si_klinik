...
$menusid = $this->M_crud->get_by_param("menu", 'name', "KodeAkun");
if(!empty($menusid)){
    		...
    		<section class="content">
    			...
    			<div class="box box-widget">
					<div class="box-header with-border">
					  <h3 class="box-title">Data Kode Akun</h3>
					</div>					
						<div class="box-body">
							<a href="<?php echo site_url('Ledger/cetak');?>" target="_blank" class="btn btn-default pull-right"><i class="fa fa-print"></i> Cetak</a>
			              	<a href="<?php echo site_url('Ledger/doexport');?>" style="margin-right: 10px;" target="_blank" class="btn btn-success pull-right">
			              		<i class="fa fa-file-excel-o"></i> Export Excell</a>
							  <?php if($this->session->userdata['simklinik']['ap_is_admin'] == 1 || $mnCreate == 1){
			              	  ?>
			              	  <a href="<?php echo base_url(); ?>Ledger/add" style="margin-right: 10px;" class="btn btn-primary pull-right">
						  ...
						  <table id="example2" class="table table-bordered table-striped">
							...
							?>
								<tr class="odd gradeX">
									<td><?= $no ?></td>
									<td><?= kode($data->kode_header,$data->kode_header_akun,$data->kode_akun) ?></td>
									<td><?= $data->nama ?></td>
									<td class="center">
							<?php
								...
								{
							?>
									<a class="btn btn-warning btn-xs" href="<?php echo site_url('Ledger/edit\/').$data->id;?>">
							...
							?>
									<a class="btn btn-danger btn-xs" title="Hapus" href="" data-toggle="modal" 
										onclick="confirm_delete('<?php echo site_url('Ledger/delete').'/'.$data->id;?>',
										'<?php echo $data->nama;?>');"><i class="fa fa-trash"></i></a>
							<?php
								}
							?>
