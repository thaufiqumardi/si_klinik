<?php
	$menusid = $this->M_crud->get_by_param("menu", 'name', "Pegawai");
	if(empty($menusid)){
    		...
    		<section class="content">
				...
				<div class="box box-widget">
					<div class="box-header with-border">
						<h3 class="box-title pull-left"> Data Pegawai</h3>
					</div>
					<div class="box-body">
						<a href="<?php echo site_url('Pegawai/cetak');?>" target="_blank" class="btn btn-default pull-right"><i class="fa fa-print"></i> Cetak</a>
			              	<a href="<?php echo site_url('Pegawai/doexport');?>" style="margin-right: 10px;" target="_blank" class="btn btn-success pull-right">
			              		<i class="fa fa-file-excel-o"></i> Export Excell</a>
							  <?php if($this->session->userdata['simklinik']['ap_is_admin'] == 1 || $mnCreate == 1){
			              	  ?>
			              	  <a href="<?php echo base_url(); ?>Pegawai/form" style="margin-right: 10px;" class="btn btn-primary pull-right">
			              	  <i class="fa fa-plus"></i> Tambah Data</a><br><br>
			              	  <?php
			              	  }
			              	  ?>
						<table id="example2" style="border: 2" class="table table-bordered table-striped table-hover DataTable" id="tablePegawai">
							<thead>
								<tr>
							...
							<tbody>
								...
								<tr>
									...
									<td><?php echo date('d-M-Y', strtotime($row['mulai_bekerja']));?></td>
									...
									<td class="center">
										<?php
											...
											{
										?>
												<a class="btn btn-warning btn-xs" href="<?php echo site_url('Pegawai/edit\/').$row['id_pegawai'];?>">
										...
										?>
												<a class="btn btn-danger btn-xs" title="Hapus" href="" data-toggle="modal" 
													onclick="confirm_delete('<?php echo site_url('Pegawai/hapus').'/'.$row['id_pegawai'];?>',
													'<?php echo $row['nama'];?>');"><i class="fa fa-trash"></i></a>
										<?php
											}
										?>
