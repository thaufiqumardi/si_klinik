					  ...
					  <h3 class="box-title">Data Hak Akses</h3>
						...
						<div class="box-body">
							  ...
							  <?php if($this->session->userdata['simklinik']['ap_is_admin'] == 1 || $mnCreate == 1){
			              	  ?>
			              	  <a href="<?php echo base_url(); ?>Previlleges/add_previlleges" style="margin-right: 10px;" class="btn btn-primary pull-right">
						  ...
						  <table id="example2" class="table table-bordered table-striped">
							...
							?>
									<tr class="odd gradeX">
										...
										<td><?= $data->title ?></td>
							<?php
										if(!empty($data->hak_akses_retrive) AND !is_null($data->hak_akses_retrive)){
											echo '<td class="text-center"><i class="fa fa-check-circle-o" style="font-size: 20px"></i></td>';
										}else{
											echo '<td></td>';
										}
										if(!empty($data->hak_akses_create) AND !is_null($data->hak_akses_create)){
											echo '<td class="text-center"><i class="fa fa-check-circle-o" style="font-size: 20px"></i></td>';
										}else{
											echo '<td></td>';
										}
										if(!empty($data->hak_akses_update) AND !is_null($data->hak_akses_update)){
											echo '<td class="text-center"><i class="fa fa-check-circle-o" style="font-size: 20px"></i></td>';
										}else{
											echo '<td></td>';
										}
										if(!empty($data->hak_akses_delete) AND !is_null($data->hak_akses_delete)){
											echo '<td class="text-center"><i class="fa fa-check-circle-o" style="font-size: 20px"></i></td>';
										}else{
											echo '<td></td>';
										}
							...
							{
							?>
								<a class="btn btn-warning btn-xs" href="<?php echo site_url('Previlleges/edit\/').$data->id_hak_akses;?>">
